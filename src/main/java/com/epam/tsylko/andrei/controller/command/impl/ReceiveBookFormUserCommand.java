...
public class ReceiveBookFormUserCommand implements Command {
    private final static Logger logger = Logger.getLogger(ReceiveBookFormUserCommand.class);
    @Override
    public String execute(String request) {
        String response;

        if (logger.isDebugEnabled()) {
            logger.debug("ReceiveBookFormUserCommand.execute()");
        }
        ...
        try {
            ...
            logger.error("request params " + ReceiveBookFormUserCommand.class.getName() + " was incorrect: " + request, e);
            response = "Incorrect request";
        } catch (ServiceException e) {
            logger.error("Error in service layer", e);
            response = "Error during operation isReturningBook ";
    ...
    public boolean getAccess(String request) {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ReceiveBookFormUserCommand.getAccess()");
        }
